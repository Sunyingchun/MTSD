...
class yolov3SPP(nn.Module):
    ...
    def __init__(self, 
        ...
        self.pyramid_pooling = pyramidPooling(1792, [6, 3, 2, 1])

        # concat+1*1 conv
        self.concatConv = conv2DBatchNorm(3584, 1024,  k_size=1, stride=1,
                                                  padding=0)
        # Final conv layers
    ...
    def forward(self, x, labels=None, segMaps=None):
        ...
        ##################PSP#########################
        up2 = F.upsample(convfinal1, conv1s.shape[2:], mode='bilinear')
        up3 = F.upsample(convfinal2, conv1s.shape[2:], mode='bilinear')
        catconv = torch.cat([up2, up3, convfinal3], 1)
        pda = self.pyramid_pooling(catconv)
